...
create table if not exists public.assigned_memberships (
  ...
  access_level public.assigned_access_level not null default 'employee',
  is_admin boolean not null default false,
  team_id uuid references public.assigned_teams (id) on delete set null,
  position_id uuid references public.assigned_positions (id) on delete set null,
  manager_user_id uuid references auth.users (id) on delete set null,
  status public.assigned_member_status not null default 'active',
  availability public.assigned_availability_status not null default 'available',
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
